...
<?php
    ...
    foreach($products as $prod) {
		foreach ($prod->skus as $k => $sku){
			if (in_array($sku->sku, $wishlistitens)) {
				$prod->sku = $sku->sku;
				$prod->originalId = $sku->originalId;
				$imageUrl = $sku->images;
				$prod->imageUrl = $imageUrl[0]->mediumImage;
				continue;
			} 
		}
		if (!isset($prod->sku)) {
...
?>
    <li data-sku="<?= $prod->sku; ?>" class="col-12 col-md-4">
        <div class="photo">
            <div class="inner">
            <?php 
